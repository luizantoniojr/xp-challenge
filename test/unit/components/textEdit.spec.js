...
test('Should show a label with the text passed by prop', () => {
    ...
    const wrapper = shallow(<TextEdit label={label}></TextEdit>);
...
test('Should show a input with the placeholder passed by prop', () => {
    ...
    const wrapper = shallow(<TextEdit placeholder={placeholder}></TextEdit>);

    expect(wrapper.find('input').prop("placeholder")).toBe(placeholder);
});

test('Should set prop id in label htmlFor', () => {
    const id = "id xp";

    const wrapper = shallow(<TextEdit id={id}></TextEdit>);
